...
export function CategoryTrends({ trends }: CategoryTrendsProps) {
  if (trends.length === 0) {
    return (
      <div className="p-4">
        <p className="text-muted">No category trends available.</p>
      </div>
    );
  }
  ...
  const maxValue = Math.max(
    ...trends.flatMap((trend) => [
      trend.spendingTrend.current,
      trend.spendingTrend.lastPeriod,
      trend.spendingTrend.samePeriodLastQuarter,
      trend.spendingTrend.samePeriodLastYear,
    ])
  );
  ...
  return (
    <div className="flex flex-col gap-25 p-4">
      {trends.map((trend) => {
        ...
        const lastYearWidth =
          (spendingTrend.samePeriodLastYear / maxValue) * 100;
          ...
          >
            ...
            <div className="flex justify-between items-baseline">
              ...
              <div className="text-2xl font-bold px-3 py-1 bg-lime-green text-primary-inverted rounded-full">
                {formatCurrency(spendingTrend.current)}
              </div>
            </div>

            {/* Period label */}
            <div className="text-xs text-card-text/40 -mt-2">
              {trend.period}
            </div>
            ...
            <div className="flex flex-col gap-2 mt-2">
              ...
              {/* Last period */}
              <div className="flex items-center gap-3">
                <div className="text-xs w-24 text-card-text/80">
                  Last period
                </div>
                <div
                  className="flex-1 relative overflow-hidden rounded-xl bg-white/20"
                ...
                {spendingTrend.changes.vsLastPeriod.percent !== 0 && (
                  <div
                    className={`flex items-center gap-1 text-xs font-semibold min-w-[50px] ${
                      spendingTrend.changes.vsLastPeriod.direction ===
                      "increase"
                        ? "text-rose"
                        : "text-lime-green"
                    }`}
                  >
                    ...
                    <span>
                      {formatPercent(
                        spendingTrend.changes.vsLastPeriod.percent
                      )}
                    </span>
                  </div>
                )}
              ...
              <div className="flex items-center gap-3">
                ...
                {spendingTrend.changes.vsLastQuarter.percent !== 0 && (
                  <div
                    className={`flex items-center gap-1 text-xs font-semibold min-w-[50px] ${
                      spendingTrend.changes.vsLastQuarter.direction ===
                      "increase"
                        ? "text-rose"
                        : "text-lime-green"
                    }`}
                  >
                    ...
                    <span>
                      {formatPercent(
                        spendingTrend.changes.vsLastQuarter.percent
                      )}
                    </span>
                  </div>
                )}
